...
vector<bool> ToDeltaE(unsigned num){
    if (num == 0) {
        cout << "There is no code for zero" << endl;
        return {};
    }
        ...
        vector<bool> vecb = ToBin(num);
        vecb.erase(vecb.begin());
        vector<bool> veca = ToGammaE(vecb.size()+1);
        vecb.insert(vecb.begin(),veca.begin(),veca.end());
        return vecb;
    }
}

unsigned DeltaETo(vector<bool> binary){
    vector<bool> vec = {0};
    if (binary == vec) {  
        cout << "There is no code for zero" << endl;
        return 0;
...
int main()
{
    ...
    cin >> numb;
    output(ToDeltaE(numb));
    cout << "Your number is: "<< DeltaETo(ToDeltaE(numb)) << endl;
    return 1;
}